	...
	return Promise.all( [
		...
		self.innerCreateIn( division, entityName + SEPARATOR + self.nodeID, async function ( message ) {
			try {
				...
				await self.innerProcessAmqp( comm )
			} catch (err) { self.logger.harconlog( err ) }
		}, false )
	] )
}
...
amqpbarrel.extendedInit = function ( config ) {
	let self = this
...
amqpbarrel.newDivision = function ( division ) {
	var self = this
	return new Promise( async (resolve, reject) => {
		try {
			if ( !self.outs[division] ) await self.createOut( division )
			resolve( 'ok' )
		} catch (err) { reject(err) }
	} )
}

amqpbarrel.removeEntity = function ( division, context, name ) {
	var self = this
	return new Promise( async (resolve, reject) => {
		if ( !self.ins[division] ) return resolve('ok')
		try {
...
amqpbarrel.newEntity = function ( division, context, name ) {
	var self = this
...
amqpbarrel.intoxicate = function ( comm ) {
	...
	if ( self.isSystemEvent( comm.event ) )
		return this.parentIntoxicate( comm )
...
amqpbarrel.setupDomains = function ( ) {
	let self = this
...
amqpbarrel.extendedClose = function ( ) {
	...
	return new Promise( async (resolve, reject) => {
		...
		try {
			if ( self.ctx )
				await promisify( self.ctx.close )
			resolve('ok')
		} catch (err) { reject(err) }
	} )
}
