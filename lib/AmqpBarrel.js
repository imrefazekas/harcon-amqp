...
function socketOutContext( ctx, socketType ){
	switch( socketType ){
...
function socketInContext( ctx, socketType ){
	switch( socketType ){
		...
		case 'PUSHWORKER': return ctx.socket( 'WORKER', {prefetch: 1} );
...
amqpbarrel.createIn = function( division, callback ){
	...
	var socket = socketInContext( self.ctx, self.socketType );
	...
	socket.on('data', function( message ){
		...
		var interested = (self.matching( reComm ).length !== 0) || self.isSystemEvent( reComm.event );
		if( !interested ) return false;
		if( self.expAck )
			socket.ack();
		self.innerProcessAmqp( comm, reComm );
	} );
	/*
...
amqpbarrel.innerProcessAmqp = function( comm, realComm ){
	var self = this;
