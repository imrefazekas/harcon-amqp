...
amqpbarrel.intoxicateMessage = async function ( comm ) {
	...
	let packet = JSON.stringify( { id: comm.id, comm: clean( comm ), callback: !!comm.callback } )
...
amqpbarrel.intoxicateAnswer = async function ( comm, err, responseComms ) {
	...
	let packet = JSON.stringify( { id: comm.id, comm: clean( comm, true ), err: error, response: true, responseComms: responseComms.map( (rc) => { return clean( rc, true ) } ) } )
